...
public final class PolenMagicController {
    ...
    private static void placeManagedLightIfPossible(PolenEntity polen, ServerLevel serverLevel) {
        ...
        BlockPos lightPos = findManagedLightPlacementSpot(polen);
        if (lightPos == null) {
            return;
        }

        serverLevel.setBlock(lightPos, ModBlocks.POLEN_LANTERN.get().defaultBlockState(), 3);
    ...
    private static BlockPos findManagedLightPlacementSpot(PolenEntity polen) {
        BlockPos origin = polen.blockPosition();
        BlockPos bestPos = null;
        int bestBrightness = Integer.MAX_VALUE;

        for (BlockPos candidate : new BlockPos[] {
                origin.north(), origin.east(), origin.south(), origin.west(),
                origin.north().east(), origin.south().east(), origin.south().west(), origin.north().west()
        }) {
            if (!PolenSafetyEvaluator.isStandableSpot(polen, candidate)
                    || !polen.level().getBlockState(candidate).isAir()) {
                continue;
            }

            int brightness = polen.level().getMaxLocalRawBrightness(candidate);
