...
public class PolenApproachTrustedPlayerGoal extends Goal {
    ...
    public boolean canUse() {
        ...
        if (player == null
                ...
                || !PolenHomeManager.isPositionWithinHomeRadius(this.polen, player.blockPosition(), HOME_SOCIAL_RADIUS)) {
            return false;
        }
    ...
    public void start() {
        ...
        PolenTaskController.markActive(this.polen, PolenTaskType.APPROACH_TRUSTED_PLAYER, "approaching_trusted_player");
    ...
    public void stop() {
        if (this.targetPlayer != null) {
            if (this.observeTicks <= 0) {
                PolenTaskController.markCompleted(
                        this.polen,
                        PolenTaskType.APPROACH_TRUSTED_PLAYER,
                        "trusted_player_reached"
                );
            } else {
